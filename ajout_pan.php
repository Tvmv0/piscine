            ...
            <div class="row">
                ...
                if ($db_found) {
                    ...
                    if (($data = mysqli_fetch_assoc($result) != 0)) {
                        echo "<h5>L'article est deja dans le panier </h5>";
                    } else {
                        //on regarde si c'est vente imédiate 
                        $sql = "SELECT * FROM items WHERE methode_vente ='1' AND id_item=$id";
                        $result = mysqli_query($db_handle, $sql);
                        if (($data = mysqli_fetch_assoc($result) == 0)) {
                            echo "<h6> l'article n'est pas en vente immédiate</h6>";
                            header("Location: parcourir.php");
                            exit;
                        } else {
                            //on regarde si l'article est en stock -----------------------------------------------************************
                            ...
                            $stock = $data['stock'];
                            if ($stock == 0) {
                                echo "<h5>l'article n'est plusdisponible </h5>";
                                echo "<br>";
                                echo "<h6>Souhaitez-vous voir à nouveau les produits disponibles ? </h6>";
                                echo '<form method="post" action="parcourir.php">';
                                echo '<button type="submit">Voir</button>';
                                echo '</form>';
                            } else {
                                echo "<h5>l'article est en stock</h5>";
                            ...
                            echo "<h4>Souhaitez-vous voir votre panier ? </h4>";
